...
function stagingName(tableName) { return `${tableName}__loading_${Date.now()}`; }
...
async function insertRows(sql, tableName, columns, rows) {
  ...
  for (let offset = 0; offset < rows.length; offset += batchSize) {
    ...
    const values = [];
    let param = 1;
    const tuples = batch.map((row) => `(${row.map((value) => { values.push(value); return `$${param++}`; }).join(', ')})`);
    const query = `INSERT INTO ${quoteIdent(tableName)} (${columnList}) VALUES ${tuples.join(', ')}`;
    try { await runWithRetry(() => sql.query(query, values)); }
    catch (error) { throw new Error(`Database request failed at row ${offset + 1} (batch=${batchSize}, columns=${columns.length}, params=${values.length}): ${error.message}`); }
  }
...
export async function appendTableRows(snapshot, rows) { return insertRows(db(), snapshot.staging, snapshot.columns, rows); }

export async function appendTableData(tableName, columns, rows) {
  if (!columns.length) throw new Error('No columns to load');
  const sql = db();
  const existing = await sql.query(`SELECT column_name FROM information_schema.columns WHERE table_schema = 'public' AND table_name = $1 ORDER BY ordinal_position`, [tableName]);
  if (!existing.length) {
    const columnSql = columns.map((name) => `${quoteIdent(name)} TEXT`).join(', ');
    ...
    if (missing.length || extra.length) throw new Error(`Append schema mismatch for ${tableName}: missing=[${missing.join(', ')}] extra=[${extra.join(', ')}]`);
  }
...
export async function abortTableSnapshot(snapshot) { await db().query(`DROP TABLE IF EXISTS ${quoteIdent(snapshot.staging)}`).catch(() => {}); }
export async function replaceTableSnapshot(tableName, columns, rows) {
  ...
  try {
    ...
    return { rowsLoaded: rows.length, columnsLoaded: columns.length, table: tableName, batchSize: result.batchSize };
  } catch (error) { await abortTableSnapshot(snapshot); throw error; }
}
